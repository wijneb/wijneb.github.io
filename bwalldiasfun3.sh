...
getWH1(){ 
    eval $( export DISPLAY=:0; export XAUTHORITY=/home/bw/.Xauthority; 
            WH=$(xrandr 2>/dev/null | grep "0.00"); 
            echo $WH | awk -Fx '{split($2, a, " "); 
            print "export WIDTH="int($1) "\nexport HEIGHT="int(a[1])}' )  
    } # makes env vars for width and hight of screenshots from within a subshell is special trick

getWH2() { eval $( export DISPLAY=:0; export XAUTHORITY=/home/bw/.Xauthority; # WH=$(xrandr 2>&1 | grep "0.00")
           xrandr 2>&1 | grep "0.00" | awk -Fx '{print "export WIDTH="int($1) "\nexport HEIGHT="int($2)}' ) ; } 

getWH(){ eval $( cat scrRes | awk -Fx '{print "export WIDTH="int($1) "\nexport HEIGHT="int($2)}' ); }
...
egfsiteok()  { egfflag=false; HE=$(showhd $egfurl); [ -z "$HE" ] && mymsg "no egf site" || egfflag=true && R=$HI; }
trmsiteok()  { trmflag=false; HT=$(showhd $trmurl); [ -z "$HT" ] && mymsg "no trm site" || trmflag=true && R=$HI; }
