...
# offset=3600  #  als uitgelezen van Oscar deze dan verwijderen

myoffset() {                                                             # offset  is bv 01:30
    hrs=$(echo $sunoff | awk -F: '{print $1}')            # 01
    min=$(echo $sunoff | awk -F: '{print $2}') 
    hrs1=$(($hrs*3600))                                            # 3600 sec
    min1=$(($min*60))                                           # 1800 sec
...
startsnapshotting() {
    while true; do
        readinifile
        myoffset           # sunset offset in secs, 1x per boot bepaalt
...
main() {  
    clockrestart       # just to be sure, restart the clock   
    startsnapshotting  # if wifi and clock are ok make snapshots (loop)
}
