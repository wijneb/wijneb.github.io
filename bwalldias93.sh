...
checksunset() {
  ...
  # time=$ss1; mytimetosecs; sss=$seconds                 # sunset 1e vd mnd in seconds 
    ssm=$(timetominutes $time)                            # sunset 1e vd maand in minutes
  # time=$sunoff; mytimetosecs; offset=$seconds           # set offset tov sunset in seconds
    offsetm=$(timetominutes $time)                        # set offset tov sunset in minutes 
  # dif1=$(($dif * 60))                                   # dif1 mnd sunset delta in sec dif is in minutes
  ...
  # mysub=$(echo "scale=2 ; $now1-$ssx" | bc)             # delta tussen tijd+offset en sunset in seconds
...
checkboottime() {
    if [ $current_hour -gt 7 ] && [ $current_hour -lt 8 ] && [ "$bootflag" = "true" ]; 
...
myruntime() { 
    current_hour=$(date +%-H)
    if [ $current_hour -gt 8 ] && [ $current_hour -lt 22 ]; 
