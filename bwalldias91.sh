...
checkdpmsoff1() {
    ...
    if [ "$current_hour" -eq 22 ] && [ "$current_minute" -lt 9 ]; then
        echo "Forcing DPMS off... at $(date +%-H):$(date +%-M)" >> mylog
       /usr/local/bin/dpmsoff
    fi
}
...
checkboottime1() {
    current_hour=$(date +%-H); current_minute=$(date +%-M) 
    if [ "$current_hour" -eq 08 ] && [ "$current_minute" -lt 9 ]; then
        echo "Rebooting...at $(date +%-H):$(date +%-M)" >> mylog
        doas /sbin/reboot
    fi
}
...
myruntime() {
    current_hour=$(date +%-H); current_minute=$(date +%-M) 
    if [ "$current_hour" -ge 08 ] && [ "$current_hour" -lt 22 ]; then
        startsnapshotting; else; sleep 540
    fi
}

mysecondsetup() {
    current_hour=$(date +%-H); current_minute=$(date +%-M)
    checkdpmsoff1; checkboottime1; myruntime
}
